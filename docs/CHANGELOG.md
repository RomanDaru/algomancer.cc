...
### `main`
- Deployed oracle support in `c3888c0`, confirmed the new production API shape, then atomically imported all 340 approved cards. Updated 285 rules revisions and flagged 174 affected decks (119 public); post-commit database comparison passed for every imported card. Rotated the catalog cache key because the public aggregate still served its pre-import cache; existing tag invalidation remains unchanged.
  - Areas: live card documents/deck review flags, `app/lib/services/cardService.ts`, `docs/oracle-review.md`. Recovery files: ignored `backups/oracle-import-1790014662930/` (`preflight.json`, `before-transaction-1.json`, `result.json`). IDs, artwork, deck contents/ownership/visibility and Game Logs preserved.
  - Verification: Vercel production deployments `c3888c0` and `24be4b1` succeeded; all 340 cards match the approved import plan through the public 511-card catalog (zero mismatches, including artwork). Read-only re-run reports zero remaining writes/revisions/deck flags. Seven cache tests passed; live Chrome checks passed for Prophecy cost 0, Spell Unit combat stats, X-cost search and mobile layout, with no page errors. Reports/screenshots are in ignored `backups/oracle-public-verification.json` and `backups/oracle-live-*.png`.
