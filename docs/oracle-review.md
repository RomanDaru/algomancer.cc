...
Import bol vykonaný 2026-09-21 po úspešnom nasadení `c3888c0`: 340 kariet, 285 zvýšených rulesVersion, 174 označených deckov (119 verejných). Kontrola databázy po transakcii prešla pre všetky karty. Záloha je `backups/oracle-import-1790014662930/`. Následne nasadený commit `24be4b1` obnovil cache kľúčom `oracle-approved-2026-09-21`; invalidačný tag `card-catalog` zostáva zachovaný. Verejné API potom potvrdilo zhodu všetkých 340 kariet vrátane obrázkov (511 kariet celkovo, nula rozdielov). Opätovný read-only import hlási nula zostávajúcich zmien. Produkčná kontrola Chrome potvrdila nulovú Prophecy cenu, combat stats Spell Unit, vyhľadávanie X a mobilné zobrazenie bez chýb stránky.
